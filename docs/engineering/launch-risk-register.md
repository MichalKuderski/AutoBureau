...
| Placeholder household data and mutations | FAIL, implementation in progress | Dashboard, item/document/obligation reads and status changes now use the tenant API locally. Household/profile/member changes persist. Notifications still use fixtures; onboarding, manual deadline creation and other actions remain incomplete. Every production path must use scoped persistence and honest empty/error states. |
