...
Latest read-only staging inspection after migration run 34715392254: 105 households / 105 entitlements, 10 completed migrations / 0 rolled back, 19 forced-RLS tables / 25 policies, no `ensure_rls` trigger. The previous 99/99 baseline gained three synthetic households from notification Preview 34712680910 and three from upload-foundation Preview 34714968801. The additive upload migration accounts for the new migration/table/policy and nullable unknown content hashes. All 20 pre-existing table counts/data fingerprints, original nine migration checksums, old grants/RLS/policies/FKs/ownership and role memberships match across DDL. The controlled identity's auth-user, session and refresh fingerprints are unchanged. The new ledger is empty, owned by postgres and forced-RLS. Main-only staging branch protection is restored. The private quarantine bucket has the verified 25 MiB/five-MIME restrictions and zero objects. No Production access was performed.
