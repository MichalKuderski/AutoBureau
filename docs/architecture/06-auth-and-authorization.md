...
- **Principal setting (ADR-009 D5).** A second transaction-local setting, `request.user_id`, read via `app.current_user_id()`. It exists because membership cannot otherwise be resolved: `household_users` is household-scoped, so a request that has not yet chosen a household sees zero rows. Requests run in two phases — phase 1 sets the principal only and may read the principal's own membership and household rows through two SELECT-only policies guarded by `app.current_household() IS NULL`; phase 2 sets the validated household, at which point those policies switch off and household isolation is the only rule in force. The principal setting grants no household data on its own.
- `audit_log`: INSERT-only for all app roles (no UPDATE/DELETE grants at all). `actor_id` defaults to `app.current_user_id()` and a check constraint requires user-attributed rows to carry one (ADR-009 D5), so invariant 9 is enforced by the database rather than by handler discipline; `actor_type='system'` rows remain insertable without a principal for background jobs.
