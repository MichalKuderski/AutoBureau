...
This proves Preview build issuance, not runtime role assumption. Stable staging's production-hosting-scope claim proof remains pending: automatic approval review rejected the proposed `--prod --skip-domain` staging-project build as a Production mutation, and that workflow was not created or run. Explicit clarification is pending. The earlier broad `env/pull` inspection was also rejected and not run; the accepted implementation reads only project/team metadata and verifies the provider-injected token inside Vercel.
