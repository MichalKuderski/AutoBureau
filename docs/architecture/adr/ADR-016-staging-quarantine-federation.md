...
Live project metadata now verifies team issuer mode and OIDC enabled for project `prj_qAjK6wDYXoGn02Sl8jjSmrvy4NLR`, team `team_CNQd2ynmaV1xtRhB6NMMeYBs`. GitHub run [34723153581](https://github.com/MichalKuderski/AutoBureau/actions/runs/34723153581) verifies the actual signature and exact Preview claims inside a native Vercel build. Its allow-listed [claim evidence](../../engineering/evidence/adr016-preview-oidc-20260912.json) contains no token. The observed token lifetime is 3,600 seconds; application upload capabilities remain capped at fifteen minutes and underlying STS expiry.

This proves Preview build issuance, not runtime role assumption. The founder subsequently explicitly approved a native `--prod --skip-domain` proof limited to the staging project, with immediate alias/protection read-back and safe removal of an unintended public proof deployment. The dedicated workflow and both identity guards are implemented. It has not run: automatic approval review separately rejected adding the temporary GitHub staging deployment rule `refs/pull/5/merge` because that protection exception was not explicitly authorized. The normal `main` rule remains intact. The earlier broad `env/pull` inspection was also rejected and not run; the accepted implementation reads only project/team metadata and verifies the provider-injected token inside Vercel.

The native AWS adapter is gated by an explicit storage-provider setting, exact staging issuer/account/region/bucket and environment-matching role. It requests temporary credentials through Vercel's documented provider and freezes the credential snapshot when deriving a URL's signature and expiry. No deployed configuration has been enabled. Remote state/deployment role, saved plan, effective-policy evidence and all real AWS provider probes remain incomplete. Terraform's verification gate stays closed.

The September 13 bootstrap draft in `infra/cloudformation/staging` prepares eight isolated state/federation resources. The state and deployment roles are separate; the latter is initially read-only, with an immutable permission ceiling for later plan-derived grants. Upload roles require a quarantine-only boundary that the deploy role cannot remove. Policies now use known exact ARNs so their trust/resource bodies are visible before apply. A Terraform precondition rejects an interactive principal even in the correct account. S3 native state locking accompanies the doc-09 DynamoDB lock because current Terraform deprecates DynamoDB locking. The [bootstrap runbook](../../../infra/cloudformation/staging/README.md) records the remaining live validation, change-set, grant, saved-plan and cleanup steps. No bootstrap change set, real plan or apply has been executed.

### September 13 live proof checkpoint
