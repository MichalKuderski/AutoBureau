...
const workflow = 'MichalKuderski/AutoBureau/.github/workflows/staging-jobs.yml@refs/pull/5/merge';
...
export async function issue(env = process.env) {
  ...
  const path = '/tmp/pellum-adr017-github-oidc.jwt';
  await writeFile(path, value, { mode: 0o600 });
  await chmod(path, 0o600);
  await writeFile('staging-jobs-oidc-proof.json', JSON.stringify(proof, null, 2), {mode:0o600});
