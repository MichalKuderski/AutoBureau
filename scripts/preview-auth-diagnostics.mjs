...
const events = ["auth.sign_up_provider_unavailable", "auth.sign_in_provider_unavailable"];
const results = events.map(event => spawnSync("vercel", ["logs", "--deployment", target, "--environment", "preview", "--since", "30m",
  "--query", event, "--json", "--limit", "20", "--token", process.env.VERCEL_TOKEN ?? ""], {
...
function inspect(value, depth = 0) {
  ...
  if (events.includes(value.event) && uuid.test(value.trace_id ?? "") && !seen.has(`${value.event}:${value.trace_id}`)) {
    seen.add(`${value.event}:${value.trace_id}`);
    ...
    process.stdout.write(JSON.stringify({ event: value.event,
      ...
      ...(["http", "timeout", "network", "invalid-response"].includes(value.meta?.upstream_failure) ? { upstream_failure: value.meta.upstream_failure } : {}),
      ...(Number.isInteger(value.meta?.upstream_duration_ms) && value.meta.upstream_duration_ms >= 0 && value.meta.upstream_duration_ms <= 600_000 ? { upstream_duration_ms: value.meta.upstream_duration_ms } : {}),
      ...(uuid.test(value.meta?.upstream_request_id ?? "") ? { upstream_request_id: value.meta.upstream_request_id } : {}),
    }) + "\n");
    found += 1;
  }
...
process.stdout.write(JSON.stringify({ diagnostic: "preview_auth", records: found, cli_succeeded: results.every(result => result.status === 0) }) + "\n");
