...
const event = { event: process.argv[process.argv.indexOf('--query')+1], trace_id: '01234567-1234-7123-8123-0123456789ab', meta: { upstream_status: 504, upstream_failure: 'http', upstream_duration_ms: 5000, upstream_request_id: '01234567-1234-4123-8123-0123456789ab', email: 'PRIVATE_CANARY@example.test' }, error_message: 'PRIVATE_CANARY', cookie: 'PRIVATE_CANARY' };
...
function run(url: string) {
  return spawnSync(process.execPath, [script, url], { encoding: "utf8", env: { ...process.env, PATH: `${fixtureDir}:${dirname(process.execPath)}:${process.env.PATH}`, VERCEL_TOKEN: "PRIVATE_CANARY" } });
}
describe("Preview diagnostic output cannot become a secret/log dump", () => {
  it("prints only the numeric provider status and validated opaque trace", () => {
    ...
    expect(result.stdout).toContain('"upstream_status":504');
    expect(result.stdout).toContain('"records":2');
    expect(result.stdout).toContain('auth.sign_in_provider_unavailable');
    expect(result.stdout).toContain('"upstream_duration_ms":5000');
